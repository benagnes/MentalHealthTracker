...
public class MoodDBhandler extends SQLiteOpenHelper {
    ...
    public String loadHandler() {
        ...
        while (cursor.moveToNext()) {
            ...
            result.append(iresult).append(": ").append(sresult).append(", ").append(s2result)
                    .append(" ").append(s3result).append(", ").append(s4result).append(", ")
                    .append(s5result).append(System.getProperty("line.separator"));
        }
        cursor.close();
        db.close();
    ...
    public boolean updateHandler(Mood mood) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(column_mood, mood.getMood());
        values.put(column_energy, mood.getEnergy());
        ...
        boolean result = (db.update(table_name, values,
                column_ID + "=" + mood.getID(), null) > 0);
        db.close();
        return result;
    }
